...
void carga_Tiles(Game *game)
{
    ...
    // fscanf a camiones a aniquilar
    fscanf(archivo, "%d", &game->contador_camiones);
    fgetc(archivo); // eof o salto linea

    // lee texto mision con fgets
    fgets(game->interfaz.texto_Mision, sizeof(game->interfaz.texto_Mision), archivo);
    int largo = strlen(game->interfaz.texto_Mision);
    if (largo > 0 && game->interfaz.texto_Mision[largo-1] == '\n')
    {
        // limpia salto linea del final linea
        game->interfaz.texto_Mision[largo - 1] = '\0';
    }
